...
const ENDPOINTS: Endpoint[] = [
    ...
    params: [
      {
        ...
        required: true,
        description:
          'ID of the completed video to publish (status must be "completed").',
      },
      {
        name: "caption",
        in: "body",
        type: "string",
        description:
          "Description/caption shared across all platforms. Max 5000 characters.",
      },
      {
        name: "publishNow",
        in: "body",
        type: "boolean",
        description:
          "true (default) to publish immediately, false to schedule.",
      },
      {
        name: "scheduledAt",
        in: "body",
        type: "string",
        description:
          "ISO 8601 datetime to schedule publication. Required when publishNow is false.",
      },
      {
        name: "channels",
        in: "body",
  ...
  },
  {
    ...
    description:
      "Returns the current status of a publication. Poll this after POST /publications until the upload reaches a stable state such as published, scheduled, or failed. Immediate publishes typically move exporting -> uploading -> published. Scheduled publishes may move into scheduled first, depending on platform behavior.",
    params: [
      {
        name: "publicationId",
        in: "path",
